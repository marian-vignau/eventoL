...
class EventTag(models.Model):
    ...
    name = models.CharField(
        _('EventTag Name'), max_length=50, unique=True, help_text=_('This name will be used as a slug')
    )
    ...
    logo_header = models.ImageField(
        null=True, blank=True, help_text=_('This logo will be shown in the right corner of the page')
    )
    logo_landing = models.ImageField(null=True, blank=True, help_text=_('Logo to show in the center of the page'))
    message = models.TextField(
        max_length=280, null=True, blank=True, help_text=_('A message to show in the center of the page')
    )
...
class Event(models.Model):
    ...
    external_url = models.URLField(
        _('External URL'), blank=True, null=True, default=None, help_text=_('http://www.my-awesome-event.com')
    )
    email = models.EmailField(verbose_name=_('Email'))
    event_information = SanitizedProseEditorField(
        verbose_name=_('Event Info'), help_text=_('Event Info HTML'), blank=True, null=True
    )
    ...
    css_custom = models.FileField(
        _('Custom CSS'), upload_to='custom_css', blank=True, null=True, help_text=_('Custom CSS file for event page')
    )
...
class ContactType(models.Model):
    ...
    validator_choices = (('1', _('Validate URL')), ('2', _('Validate Email')), ('3', _("Don't validate")))
    ...
    validate = models.CharField(
        _('Level'), choices=validator_choices, max_length=10, help_text=_('Type of field validation')
    )
...
class Contact(models.Model):
    ...
    event = models.ForeignKey(
        Event, verbose_name=_noop('Event'), related_name='contacts', blank=True, null=False, on_delete=models.CASCADE
    )
...
class EventUserManager(models.Manager):
    ...
    def get_counts(event_users):
        confirmed = (
            EventUserAttendanceDate.objects.filter(event_user__in=event_users).order_by('event_user').distinct().count()
        )
...
class EventUserAttendanceDate(models.Model):
    ...
    event_user = models.ForeignKey(
        EventUser, verbose_name=_noop('Event User'), blank=False, null=False, on_delete=models.CASCADE
    )
...
class Collaborator(models.Model):
    ...
    updated_at = models.DateTimeField(_('Updated At'), auto_now=True)
    event_user = models.ForeignKey(
        EventUser, verbose_name=_('Event User'), blank=True, null=True, on_delete=models.CASCADE
    )
...
class AttendeeManager(EventUserManager):
    ...
    def get_counts(self, queryset):
        ...
        confirmed_with_event_user = (
            AttendeeAttendanceDate.objects.filter(attendee__event_user__in=event_users)
        ...
        confirmed = (
            AttendeeAttendanceDate.objects.filter(attendee__in=attendees, attendee__event_user__isnull=True)
            .order_by('attendees')
            .distinct()
            .count()
        )
        ...
        return {
            ...
            'without_event_user': {'total': total, 'confirmed': confirmed, 'not_confirmed': total - confirmed},
        }
...
class AttendeeAttendanceDate(models.Model):
    ...
    attendee = models.ForeignKey(
        Attendee, verbose_name=_noop('Attendee'), blank=False, null=False, on_delete=models.CASCADE
    )
    date = models.DateTimeField(_('Date'), help_text=_('The date of the attendance'), auto_now_add=True)
    attendance_mode_choices = (
        ('1', _('Qr autoregistration')),
...
class InstallationMessage(models.Model):
    ...
    message = SanitizedProseEditorField(
        verbose_name=_('Message Body'), help_text=_('Email message HTML Body'), blank=True, null=True
    )
...
class Installer(models.Model):
    objects = EventUserManager()
    installer_choices = (('1', _('Beginner')), ('2', _('Medium')), ('3', _('Advanced')), ('4', _('Super Hacker')))
    created_at = models.DateTimeField(_('Created At'), auto_now_add=True)
    updated_at = models.DateTimeField(_('Updated At'), auto_now=True)
    event_user = models.ForeignKey(
        EventUser, verbose_name=_('Event User'), blank=True, null=True, on_delete=models.CASCADE
    ...
    level = models.CharField(
        _('Level'), choices=installer_choices, max_length=200, help_text=_('Knowledge level for an installation')
    )
...
class Activity(VoteModel, models.Model):
    ...
    justification = models.TextField(
        _('Justification'), blank=True, null=True, help_text=_('Why do you reject this proposal?')
    )
    ...
    speaker_bio = models.TextField(
        _('Speaker Bio'), null=True, help_text=_('Tell us about you (we will use it as your "bio" in our website)')
    )
    ...
    external_link = models.URLField(
        ...
        null=True,
        max_length=500,
        help_text=_('Link to presentation, video, blog, or other external resource'),
    )
    level_choices = (
        ('1', _('Beginner')),
        ('2', _('Medium')),
    ...
    status = models.CharField(
        _('Status'), choices=status_choices, max_length=20, help_text=_('Activity proposal status')
    )

    image = ImageCropField(
        upload_to='images_thumbnails',
    ...
    def room_available(cls, request, proposal, event_slug, event_date, error=False):
        ...
        if (
            ...
            .exists()
            or activities_room.filter(end_date__gt=proposal.end_date, start_date__lt=proposal.start_date)
            .exclude(pk=proposal.pk)
            .exists()
            or activities_room.filter(
                start_date__range=(proposal.start_date + one_second, proposal.end_date - one_second)
            )
            .exclude(pk=proposal.pk)
            .exists()
            or activities_room.filter(end_date=proposal.end_date, start_date=proposal.start_date)
            .exclude(pk=proposal.pk)
            .exists()
        ):
            message = _(
